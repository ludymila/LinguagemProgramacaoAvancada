...
int pegaParametro(int qtdParam){
	int casaDecimal=0; 	param[0] =0;param[1] =0; param[2] =0; param[3] =0; 
	int i = 0;
	printf("cont:%c \n", comandos[cont]);
	...
	for (i=0;i<qtdParam; i++){
		while (comandos[cont] != ' '){
			param[i] += ((comandos[cont] - '0') * (10 ^ (casaDecimal)));
			casaDecimal++;
			cont++;
		}
		printf("%d \n", param[i]);
...
int main(int argc, char *argv[]){
   ...
   cont = 1;
	...
	int aux=0;
    printf("este: %c", comandos[cont]);
	printf("esteoutro: %c", comandos[cont+1]);
	
	aux = 1;
	//cont ta com a posicao do comando
	while(comandos[cont+1] != '_') {
		printf("entrou \n");
		...
		case 'I': printf("oii"); pegaParametro(2);
				I(param[0], param[1]); C(); S();
			break;
			/*case 'I': I(comandos[cont], comandos[5]);
			break;
			case 'C': C();
			break;
