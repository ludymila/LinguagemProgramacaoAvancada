...
puts(comandos);
	...
	while(comandos[cont] != '_') {
		...
		case 'I':  pegaParametro(2); if (!sair) {I(param[0], param[1]); C(); S();}
			break;
			
			case 'C': C();cont=cont+1;
			break;
			case 'L': pegaParametro(2); if (!sair) {L(param[0], param[1], comandos[cont]); cont = cont+2; S();}
			break;
			case 'V': pegaParametro(3); if (!sair) {V(param[0], param[1], param[2], comandos[cont]); cont = cont+2; S();}
			break;
			case 'H': pegaParametro(3); if (!sair) {H(param[0], param[1], param[2], comandos[cont]); cont = cont+2; S();}
			break;
			case 'K': pegaParametro(4); if (!sair){K(param[0], param[1], param[2], param[3], comandos[cont]); cont = cont+2; S();}
			break;
			case 'F': pegaParametro(2); if (!sair){F(param[0], param[1], comandos[cont]); cont= cont+2; S();}
			break;
