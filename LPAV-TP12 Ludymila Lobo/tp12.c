...
void printPermutacao(char v[], const int size){
	int i;	
	if (v != NULL) {
		for (i=0;i<size; i++){
			printf("%c", v[i]);
...
int contUnico=0;
void printCombinacao(char* v, const int size, int repeticao){
	const int tam = repeticao;
	int i;
	const int tam2 = size;
	char combinacoes[tam];
	int aux[tam2];	
	int iguais;
	iguais=0;
	if (v !=0) {
...
void permutacao(char* v, const int start, const int n, int repeticao){
	int i;	
	if (start == n-1){
...
void questao6(char* vet, int tam){
	//int v[] = {1,2,3,4};
	int i;
	//printf("na 6\n");
	//for (i=0;i<tam-2;i++) {
