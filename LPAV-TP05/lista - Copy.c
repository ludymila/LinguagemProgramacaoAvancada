...
int main(int argc, char *argv[]){
	...
	{  
		...
		if (lista!= NULL) eventoAnterior = eventoNovo; //valor anterior
		eventoNovo   = (evento_t*)        malloc (sizeof (evento_t));
		eventoAnterior   = (evento_t*)        malloc (sizeof (evento_));
		//salvando as informacoes no struct evento
		eventoNovo->num = num;
		
	if (lista == NULL) {
		primeiro = eventoNovo;
		eventoNovo->anterior = NULL;
	}
		
		else eventoNovo->anterior = eventoAnterior;
