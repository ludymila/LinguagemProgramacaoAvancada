...
int *vector; 
	...
	printf("\n0-Soma de vetores COM THREAD\n");
	printf("1-Calculo de multiplicacao SEM THREAD\n");
	printf("2-Calculo de multiplicacao SEM THREAD\n");
	printf("3-Calculo de multiplicacao COM THREAD\n");
	...
	printf("Tamanho do vetor");
		...
		int *vetor; 
			...
			scanf("%d", &n);
			amount_of_elements_in_run = n;
			vetor = generate_random_vector(n);

			printf("* Soma dos elementos do vetor sem thread: %d\n",calculate_sum_vector_in_main(vetor,n,&spent_time));
			printf("* Tempo decorrido (microssegundos): %lf\n\n",spent_time);
	}
	else if (opcao ==2){
			...
			printf("* Soma dos elementos da matriz sem thread: %d\n",calculate_sum_matriz_in_main(matriz,n,&spent_time));
			printf("* Tempo decorrido (microssegundos): %lf\n\n",spent_time);
			/*
			calculate_sum_vector_with_k_threads(vector,n,k,&spent_time2);
			printf("* Soma dos elementos do vetor Com k(%d)-Threads: %d\n",k,vector[0]);
		...
		if ((n < 1) || (n > MAX_THREAD))
		{
			printf("O numero de threads deve ser entre 1 e %d.\n", MAX_THREAD);
			exit(1);
		}
