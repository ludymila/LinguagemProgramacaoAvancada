...
bool somaBinario(){
	//int p=0; // @pauli nao usou p na somabinario
	binPosicao = 6; //sempre comeca a somar da ultima posicao //@pauli
	//p=qtdVazia; // @pauli nao usou p na somabinario
...
void preencheforcaBruta(){
	int i,j, p=0; // @pauli nao usou i e j no preencheforcabruta
	...
	do{	
		...
		if (validaMatriz()) {
			...
			break;
				...
				for(p=0;p<qtdVazia;p++){ // @pauli sao apenas 6
					printf("binario: p %d \n", binario[p]);
                    somaBinario(); // moved by @pauli
                    
				}
           
			// somaBinario();      // removed  by @pauli
		}
